...
func (c *Coordinator) DoScrape(ctx context.Context, r *http.Request) (*http.Response, error) {
	...
	// asgInstanceID and wrapperName are under our control,
	// so we can enforce that they cannot have dots
	// But serviceName can be an autogenerated cname (as in case of aptible)
	// and it can have dots.  So we have to split the following way
	...
	if len(b) >= 3 {
		...
		level.Error(c.logger).Log("msg", "unsupported scraping endpoint", serviceWrapper)
	}
